...
const AccountantAttendance = () => {
  ...
  const accountantName = localStorage.getItem("AccountantName") || "Accountant";

  useEffect(() => {
    const saved = localStorage.getItem("Accountant-attendance");
    if (saved) setAttendanceRecords(JSON.parse(saved));

    const savedRequests = localStorage.getItem("Accountant-leave-requests");
    if (savedRequests) setLeaveRequests(JSON.parse(savedRequests));
  }, []);

  useEffect(() => {
    localStorage.setItem("Accountant-attendance", JSON.stringify(attendanceRecords));
    localStorage.setItem("accountant-leave-requests", JSON.stringify(leaveRequests));
  ...
  const getYearlySummary = () => {
    const currentYear = String(new Date().getFullYear()); // ✅ fix
    const summary = { present: 0, absent: 0, leave: 0 };
    for (const date in attendanceRecords) {
  ...
  midnightToday.setHours(0, 0, 0, 0); // ✅ for comparisons
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-gray-100 to-white dark:from-gray-900 dark:to-gray-800 text-gray-800 dark:text-white">
      <h2 className="text-3xl font-bold mb-6">{accountantName}'s Attendance </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg">
          <Calendar
            value={selectedDate}
            minDate={new Date()} // keep: block past
            onChange={setSelectedDate}
            className="w-full rounded-lg overflow-hidden" // wrapper class
            tileContent={({ date }) => (
            ...
            tileClassName={({ date }) => {
              ...
              const isPast = date < midnightToday; // ✅ compare Date objects
