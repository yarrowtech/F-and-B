...
export default function AccountantOrderBilling() {
  ...
  const [quickNameSearchOpen, setQuickNameSearchOpen] = useState(false);
  ...
  const quickNameSearchResults = quickNameSearch.trim()
    ? menuItems
        .filter(
          (item) =>
            ...
            String(item.name || "")
              .toLowerCase()
              .includes(quickNameSearch.trim().toLowerCase())
        )
        .slice(0, 8)
    : [];
            ...
            {tab !== "NEW" && (
              <div className="flex min-h-10 w-full items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 dark:border-slate-700 dark:bg-slate-950 lg:max-w-md">
                ...
                <input
                  ...
                  className="h-10 w-full bg-transparent text-sm text-slate-700 outline-none placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500"
                ...
                <div className="rounded-[24px] border border-[#d7e3ef] bg-white p-3.5 shadow-sm dark:border-slate-700 dark:bg-slate-950 min-[1024px]:p-4 xl:rounded-[26px] xl:p-5">
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-3">
                    <div className="rounded-xl bg-white px-3 py-3 ring-1 ring-[#d7e3ef] dark:bg-slate-900 dark:ring-slate-700 xl:rounded-2xl xl:px-4 xl:py-4">
                      ...
                      <p className="mt-1 truncate text-xs font-black uppercase tracking-wide text-slate-800 dark:text-slate-100">
                    ...
                    <div className="rounded-xl bg-white px-3 py-3 ring-1 ring-[#d7e3ef] dark:bg-slate-900 dark:ring-slate-700 xl:rounded-2xl xl:px-4 xl:py-4">
                      ...
                      <p className="mt-1 truncate text-xs font-black tracking-wide text-slate-800 dark:text-slate-100">
              ...
              <div className="rounded-[24px] border border-[#d7e3ef] bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-950 xl:rounded-[26px] xl:p-5">
                ...
                <div className="relative mb-4 rounded-[22px] border border-emerald-200 bg-[#f2fcf7] p-4 dark:border-emerald-900/40 dark:bg-emerald-950/20 xl:rounded-[24px] xl:p-5">
                  ...
                  >
                    ...
                    <p className="mt-1.5 text-xs font-medium text-emerald-700/80 dark:text-emerald-200/80 xl:mt-2 xl:text-sm">
                      Type the item name and tap a result to add it.
                    </p>
                    <input
                      type="text"
                      autoComplete="off"
                      value={quickNameSearch}
                      onChange={(e) => {
                        setQuickNameSearch(e.target.value);
                        setQuickNameSearchOpen(true);
                      }}
                      onFocus={() => setQuickNameSearchOpen(true)}
                      placeholder="Search item name, e.g. Paneer Tikka"
                      className="mt-3 min-h-12 w-full rounded-xl border border-emerald-200 bg-white px-4 text-sm font-semibold text-slate-800 outline-none focus:border-emerald-500 dark:border-emerald-900/50 dark:bg-slate-900 dark:text-slate-100 xl:min-h-14 xl:rounded-2xl xl:px-5 xl:text-base"
                      {...getTouchInputProps("text")}
                    />

                    {quickNameSearchOpen && quickNameSearch.trim() && (
                      <div className="absolute left-0 right-0 top-full z-30 mt-2 max-h-72 overflow-y-auto rounded-2xl border border-emerald-200 bg-white shadow-xl dark:border-emerald-900/50 dark:bg-slate-900">
                        {quickNameSearchResults.length === 0 ? (
                          <p className="px-4 py-3 text-sm font-semibold text-slate-500">
                            No matching items
                          </p>
                        ) : (
                          quickNameSearchResults.map((item) => (
                            <button
                              ...
                              className="flex min-h-12 w-full items-center justify-between gap-3 border-b border-slate-100 px-4 py-2.5 text-left last:border-b-0 hover:bg-emerald-50 dark:border-slate-800 dark:hover:bg-emerald-950/20"
                            >
                ...
                <button
                  ...
                  className="mt-4 hidden min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <FaReceipt />
              ...
              <div className="hidden min-[1024px]:block min-[1024px]:sticky min-[1024px]:top-4 min-[1024px]:self-start">
                <div className="rounded-[24px] border border-[#d7e3ef] bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-950 xl:rounded-[26px] xl:p-5">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowManualAdvanced((value) => !value)}
                    className="mt-5 inline-flex min-h-10 w-full items-center justify-center rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
                  >
                    {showManualAdvanced ? "Hide Taxes & Fees" : "Edit Taxes & Fees"}
                  </button>
                  ...
                  <button
                    ...
                    onClick={submitManualBill}
                    disabled={creatingManualBill || manualBill.items.length === 0}
                    className="mt-5 inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-base font-black text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    <FaCheck />
                    {creatingManualBill ? "Generating..." : "Finalize Order"}
                  </button>
                </div>
              </div>
            </div>
