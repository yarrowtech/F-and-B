// src/SucheifModule/SucheifPersonalNotes.jsx
import React, { useState, useEffect } from "react";
import { FaPlus, FaTrash } from "react-icons/fa";

const SucheifNotes = () => {
  const [notes, setNotes] = useState(() => {
    const saved = localStorage.getItem("sucheifPersonalNotes");
    return saved ? JSON.parse(saved) : [];
  });
  const [input, setInput] = useState("");

  useEffect(() => {
    localStorage.setItem("sucheifPersonalNotes", JSON.stringify(notes));
  }, [notes]);
    ...
    <div className="p-8 bg-gray-50 dark:bg-neutral-900 min-h-screen">
      ...
      <div className="space-y-4">
        {notes.map((note) => (
          ...
          >
            <span className="text-gray-800 dark:text-gray-100 text-md">
              {note.text}
            </span>
            <button
