...
const SUcheifAttendancePage = () => {
  ...
  const suCheifName = localStorage.getItem("SucheifName") || "SuCheif";

  useEffect(() => {
    const saved = localStorage.getItem("su-cheif-attendance");
    if (saved) setAttendanceRecords(JSON.parse(saved));

    const savedRequests = localStorage.getItem("su-cheif-leave-requests");
    if (savedRequests) setLeaveRequests(JSON.parse(savedRequests));
  }, []);

  useEffect(() => {
    localStorage.setItem("su-cheif-attendance", JSON.stringify(attendanceRecords));
    localStorage.setItem("su-cheif-leave-requests", JSON.stringify(leaveRequests));
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-gray-100 to-white dark:from-gray-900 dark:to-gray-800 text-gray-800 dark:text-white">
      <h2 className="text-3xl font-bold mb-6">{suCheifName}'s Attendance</h2>
...
export default SUcheifAttendancePage;
