...
export default function sucheifLogin() {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    // Generate name from sucheifId
    const generateName = (id) => {
      const namePart = id.replace(/\./g, ' ');
      return namePart.charAt(0).toUpperCase() + namePart.slice(1);
    };
    const name = generateName(formData.sucheifId);
    localStorage.setItem('sucheifName', name);
    localStorage.setItem('sucheifLoginId', formData.sucheifId);
    alert(`Su-chef Login: ${formData.sucheifId}`);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-lime-100 via-green-100 to-lime-200 flex items-center justify-center px-4">
      <div className="relative w-full max-w-md p-8 backdrop-blur-xl bg-white/40 border border-white/50 rounded-3xl shadow-2xl">
        ...
        <h2 className="text-2xl font-bold text-center text-green-900 drop-shadow mb-6 mt-2">
          Su-cheif {activePage === "login" ? "Login" : "Reset Password"}
        </h2>
        ...
        {activePage === "login" && (
          <form onSubmit={handleLogin} className="space-y-4">
            <input
              ...
              type="text"
              name="sucheifId"
              placeholder="Su-cheif ID"
              value={formData.sucheifId}
