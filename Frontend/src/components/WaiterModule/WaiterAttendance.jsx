...
const WaiterAttendancePage = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("Waiter-attendance");
    if (saved) setAttendanceRecords(JSON.parse(saved));

    const savedRequests = localStorage.getItem("Waiter-leave-requests");
    if (savedRequests) setLeaveRequests(JSON.parse(savedRequests));
  }, []);

  useEffect(() => {
    localStorage.setItem("Waiter-attendance", JSON.stringify(attendanceRecords));
    localStorage.setItem("Waiter-leave-requests", JSON.stringify(leaveRequests));
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-gray-100 to-white dark:from-gray-900 dark:to-gray-800 text-gray-800 dark:text-white">
      <h2 className="text-3xl font-bold mb-6">{waiterName}'s Attendance </h2>
