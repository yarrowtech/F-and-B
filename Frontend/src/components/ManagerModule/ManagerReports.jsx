...
const getInventoryReports = (sections = []) => [
...
export default function ManagerReports() {
  ...
  const openReport = async (input) => {
    ...
    const definition =
      typeof input === "object" && input?.key
        ? input
        : catalog.find((item) => item.title === input) || {
            key: getReportKey(input),
            title: input,
          };
  ...
  const inventoryGroup = useMemo(
    () => ({
      key: "inventory",
      label: "Inventory",
      icon: FileSpreadsheet,
      reports: getInventoryReports(kitchenSections),
    }),
    [kitchenSections]
  );
    ...
    return allGroups
      ...
      .map((group) => ({
        ...
        reports: group.reports.filter((report) =>
          String(typeof report === "string" ? report : report.title)
            .toLowerCase()
            .includes(query)
        ),
      }))
      .filter((group) => group.reports.length > 0);
  }, [activeFilter, allGroups, search]);
