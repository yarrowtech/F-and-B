    ...
    <article className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-slate-200 dark:bg-neutral-900 dark:ring-neutral-700">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 rounded-lg bg-slate-50 p-2.5 text-center dark:bg-neutral-800">
        ...
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-slate-900 dark:text-white">{formatQuantity(item.lowStockThreshold)}</p>
        </div>
      </div>
...
const InventoryManagerManagement = () => {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      if (updated) {
        setInventory((p) => p.map((i) => i._id === editingId ? updated : i));
  ...
  /* ── shared category select props ── */
  const catProps = {
    allCategories: categories,
    customValue: catCustom,
    onChange: (v) => { setForm((f) => ({ ...f, category: v })); if (v !== "__new__") setCatCustom(""); },
    onCustomChange: setCatCustom,
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-2 pb-28 dark:bg-neutral-950 sm:p-4 sm:pb-32 2xl:p-5">
          ...
          {restaurantId && (
            <div className="grid gap-2 sm:grid-cols-[minmax(136px,auto)_minmax(136px,auto)_auto_auto] sm:items-center sm:justify-end">
              <div className="space-y-1">
                ...
                />
                <p className="text-xs text-slate-500 dark:text-neutral-400">{formatDateTimePreview(exportFrom)}</p>
              </div>
              <div className="space-y-1">
                <input
                  type="datetime-local"
                  ...
                  className="min-h-10 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white"
                />
                <p className="text-xs text-slate-500 dark:text-neutral-400">{formatDateTimePreview(exportTo)}</p>
              </div>
              <button
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                {!search && (
                  <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">
                    Click <span className="font-semibold text-green-600">+ Add Item</span> to get started
                  </p>
                )}
              </div>
            ) : (
        ...
        <Modal title={`Logs — ${logsItemName}`} accent="bg-gradient-to-r from-blue-600 to-blue-500"
          ...
          {logs.length === 0 ? (
            <div className="flex flex-col items-center py-10">
              ...
              <p className="text-gray-400 dark:text-gray-500 font-medium">No logs found.</p>
            </div>
          ) : (
        ...
        <Modal title="Add Inventory Item" accent="bg-gradient-to-r from-green-600 to-emerald-500" onClose={() => setShowAddModal(false)}>
          <form onSubmit={handleAdd} className="space-y-4">
            <Field label="Item Name">
              <input type="text" placeholder="e.g. Tomato" value={form.name}
                onChange={(e) => setForm({ ...form, name: e.target.value })}
                required className={inputCls} />
            </Field>
            <CategorySelect value={form.category} {...catProps} />
            <UnitSelect value={form.unit} customValue={form.unitCustom}
              onChange={(v) => setForm({ ...form, unit: v, unitCustom: "" })}
              onCustomChange={(v) => setForm({ ...form, unitCustom: v })} required />
            <div className="grid grid-cols-2 gap-4">
              <Field label="Initial Quantity">
                <input type="number" min="0" step="any" placeholder="e.g. 50" value={form.quantity}
                  onChange={(e) => setForm({ ...form, quantity: e.target.value })} required className={inputCls} />
              </Field>
              <Field label="Low Stock Alert">
                <input type="number" min="0" step="any" placeholder="e.g. 10" value={form.lowStockThreshold}
                  onChange={(e) => setForm({ ...form, lowStockThreshold: e.target.value })} required className={inputCls} />
              </Field>
            </div>
      ...
      {/* ════════ EDIT ITEM MODAL ════════ */}
      {showEditModal && (
        <Modal title="Edit Inventory Item" accent="bg-gradient-to-r from-amber-500 to-orange-500"
          ...
          <form onSubmit={handleEdit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => { setShowEditModal(false); setEditingId(null); }}
                className="flex-1 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                Cancel
              </button>
              <button type="submit" disabled={submitting}
                className="flex-1 bg-amber-500 hover:bg-amber-600 text-white py-2.5 rounded-xl text-sm font-semibold disabled:opacity-60 transition-colors">
      ...
      {showAddStockModal && stockTarget && (
        <Modal title="Update Stock" accent="bg-gradient-to-r from-emerald-600 to-green-500"
          onClose={() => { setShowAddStockModal(false); setStockTarget(null); }}>
          <form onSubmit={handleAddStock} className="space-y-4">
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/40 rounded-xl p-4">
              ...
              {stockTarget.category && (
                <span className="inline-flex mt-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
                  {stockTarget.category}
                </span>
              )}
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Current stock: <span className="font-semibold text-emerald-700 dark:text-emerald-400">{formatQuantity(stockTarget.quantity)} {stockTarget.unit}</span>
              </p>
            </div>
            ...
            <Field label={stockMode === "add" ? `Quantity to Add (${stockTarget.unit})` : `Current Stock Quantity (${stockTarget.unit})`}>
              <input type="number" min="0" step="any" placeholder={stockMode === "add" ? "e.g. 20" : "e.g. 85"}
            ...
            <div className="-mx-5 sticky bottom-[72px] grid grid-cols-2 gap-3 border-t border-gray-100 bg-white px-5 pb-3 pt-3 dark:border-gray-700 dark:bg-gray-900 sm:-mx-6 sm:bottom-0 sm:px-6">
              ...
              <button type="submit" disabled={submitting}
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl text-sm font-semibold disabled:opacity-60 transition-colors">
                {submitting ? "Saving..." : stockMode === "add" ? "Add Stock" : "Save Stock"}
              </button>
            </div>
          </form>
        </Modal>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 space-y-2 text-sm">
              <div className="flex justify-between text-gray-600 dark:text-gray-300">
                <span>Unit</span><span className="font-semibold">{deleteTarget.unit}</span>
              </div>
              <div className="flex justify-between text-gray-600 dark:text-gray-300">
                <span>Quantity</span><span className="font-semibold">{formatQuantity(deleteTarget.quantity)}</span>
              </div>
              <div className="flex justify-between text-gray-600 dark:text-gray-300">
                <span>Low Stock Threshold</span><span className="font-semibold">{formatQuantity(deleteTarget.lowStockThreshold)}</span>
              </div>
            </div>
...
export default InventoryManagerManagement;
