...
const paymentHistory = [
  { month: "Jan", year: 2025, basic: 45000, allowances: 8000, deductions: 4000 },
  { month: "Feb", year: 2025, basic: 45000, allowances: 8500, deductions: 4000 },
  { month: "Mar", year: 2025, basic: 45000, allowances: 9000, deductions: 4000 },
];

const attendanceData = [
  { month: "Jan", year: 2025, present: 25, absent: 3, leaves: 2 },
  { month: "Feb", year: 2025, present: 24, absent: 4, leaves: 2 },
  { month: "Mar", year: 2025, present: 27, absent: 3, leaves: 0 },
];
...
const InventoryManagerProfile = () => {
  ...
  const paymentWithNet = useMemo(
    () => paymentHistory.map((p) => ({ ...p, net: p.basic + p.allowances - p.deductions })),
    []
  );
  const latestPayment = paymentWithNet[paymentWithNet.length - 1];
  const yearlyNet = useMemo(() => paymentWithNet.reduce((acc, p) => acc + p.net, 0), [paymentWithNet]);
      ...
      <div className="overflow-x-auto rounded-xl border bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
        <table className="min-w-full text-left">
          ...
          <tfoot className="bg-neutral-50 text-sm dark:bg-neutral-700/50">
            <tr>
              <td className="px-4 py-2 font-medium" colSpan={5}>Total (Net)</td>
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "employeeId": return renderEmployeeId();
      case "payment": return renderPayment();
      case "employeeDetails": return renderEmployeeDetails();
    ...
    <div className="min-h-screen bg-gray-50 p-4 dark:bg-gray-900 sm:p-6 print:bg-white">
      <header className="sticky top-0 z-10 mb-4 border-b bg-gray-50/80 backdrop-blur dark:border-neutral-800 dark:bg-gray-900/80 print:static print:border-none print:bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between py-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Inventory Manager Profile</h1>
        ...
        <nav
          role="tablist"
          aria-label="Inventory Manager profile sections"
