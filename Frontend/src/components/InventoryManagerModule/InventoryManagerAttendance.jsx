...
const InventoryManagerAttendancePage = () => {
  ...
  const managerName = localStorage.getItem("InventoryManagerName") || "Inventory Manager";

  useEffect(() => {
    const saved = localStorage.getItem("inventory-manager-attendance");
    if (saved) setAttendanceRecords(JSON.parse(saved));

    const savedRequests = localStorage.getItem("inventory-manager-leave-requests");
    if (savedRequests) setLeaveRequests(JSON.parse(savedRequests));
  }, []);

  useEffect(() => {
    localStorage.setItem("inventory-manager-attendance", JSON.stringify(attendanceRecords));
    localStorage.setItem("inventory-manager-leave-requests", JSON.stringify(leaveRequests));
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-gray-100 to-white dark:from-gray-900 dark:to-gray-800 text-gray-800 dark:text-white">
      <h2 className="text-3xl font-bold mb-6">{managerName}'s Attendance </h2>
