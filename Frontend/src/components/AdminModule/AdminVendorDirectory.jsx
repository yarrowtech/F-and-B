...
function VendorTypeBadge({ vendor }) {
  if (vendor?.loginAccess === "not_required") {
    return (
      <span className="inline-flex items-center rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-semibold text-sky-700 dark:bg-sky-900/40 dark:text-sky-300">
        Manual
      </span>
    );
  }
  ...
  return (
    <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-700 dark:bg-neutral-700 dark:text-slate-200">
      Local
    </span>
  );
}
